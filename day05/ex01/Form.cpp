...
Form::Form(const std::string name, bool bl, const int g_sig, const int g_exec) : name(name), g_signe(g_sig), g_execute(g_exec)
{
    f_signe = bl;
}
...
bool        Form::getF_signe(void)
{
    return this->f_signe;
}

int         Form::getG_signe(void)
{
    return this->g_signe;
}

int         Form::getG_execute(void)
{
